...
- [24. 最终知识地图](#24-最终知识地图)
- [25. 秋招项目竞争力与优化路线](#25-秋招项目竞争力与优化路线)
- [附录 A：术语表](#附录-a术语表)
- [附录 B：命令速查](#附录-b命令速查)
...
主题编译补充了 `PageCitation.section`：原文事实可按设计原因、当前规则、历史变化等
知识章节组织，而不是按来源文件排列。包含这类引用的页面记录 `generated: topic_wiki`。
`ingest --llm --reorganize` 可选择已经发布的来源重新组织；普通增量编译提供同一主题的
完整当前来源及旧草稿，允许新来源加入一个现有主题，但不允许偷移其他页面的来源。
当前仍采用一个来源对应一个页面归属的约束，尚不支持同一来源同时作为多个主题的所有者。

`query/topic_context.py` 检查主题全部输入的当前版本、发布版本和文件依赖状态。
综合问题可以复用未过期的模型说明，但说明仍是未验证的组织提示，不是新增事实。
模型输入同时包含分章节的原文引用；任一来源过期后，不再发送该主题说明。
`ask --llm` 可从主题对应的已发布原文补充细节；综合回答不足时最多追加一次原文回查，
因此可能多一次模型调用。原文回查单独标注 `source_passage`，没有改变 MCP 只接受已发布
Wiki Fact 的证据读取合同。模型说明是否准确仍需要人工审核或独立语义评测。
...
**MemoryForge 不普通，但现在的问题不是技术不够，而是技术价值太分散。**

如果只说“本地知识库、RAG、MCP”，它会落入常见 AI 项目叙事；如果用“版本化证据、
不可变提案、哈希绑定审核、跨介质恢复、证据充分性门禁”讲清主线，它是一个有系统设计
深度的 AI 基础设施项目。

下面是面向后端、AI Infra、开发工具岗位的主观评分。评分评价的是“招聘者能否快速识别
能力”，不是代码质量认证。

| 维度 | 当前判断 | 原因 |
| --- | ---: | --- |
| 问题定义 | 8.5 / 10 | 解决知识过期、不可审核、不可追溯，问题真实且边界明确 |
| 架构深度 | 9 / 10 | SourceVersion、ChangeSet、Receipt、Journal、Git、SQLite 形成完整状态链 |
| 工程可靠性 | 8.5 / 10 | 有类型、Lint、测试、clean-room 构建、失败恢复和发布 Evidence |
| 检索与算法 | 7 / 10 | FTS5、RRF、Support Score 可解释，但语义泛化能力有限 |
| 产品完成度 | 7.5 / 10 | CLI、Portal、Desktop、MCP 都可用，但入口较多、学习成本偏高 |
| 外部有效性 | 5.5 / 10 | 有公开 Benchmark 和负结果，但真实用户、真实长期使用数据不足 |
| 可维护性 | 5.5 / 10 | 核心模块偏大，历史实验与制品较多，阅读和评审成本高 |
| 首屏表达 | 6.5 / 10 | README 完整，但“最难的工程问题”没有在一分钟内完全凸显 |

**总体判断：可以作为秋招主项目。** 当前更像“技术很强但需要讲解员”的项目；优化目标
不是再加功能，而是让陌生面试官在 5 分钟内看到问题、取舍、证据和结果。

### 25.2 为什么它不是普通套壳项目

| 常见 Demo | MemoryForge 的工程工作 |
| --- | --- |
| 调一个模型 API 返回文本 | 模型只生成候选，程序验证 Citation 和发布边界 |
| 文档切片后直接入库 | 原文先形成内容寻址的 SourceVersion 和 Manifest |
| 更新后直接覆盖索引 | ChangeSet、ReviewReceipt、ApprovalReceipt、Apply 分离 |
| 只处理成功路径 | Journal、Git 基线、来源版本和 fail-closed 恢复处理失败路径 |
| 只比较回答样例 | 固定题集、Raw FTS 基线、负结果和 Evidence Registry |
| 画一张模型生成的架构图 | Tree-sitter 抽取 Symbol/Relation，再生成可回源的架构边 |
| 每个 AI 客户端单独接一遍 | 多客户端复用本地 MCP Router 和统一权限合同 |

真正能证明能力的不是“功能数量”，而是以下四个问题都有代码答案：

1. **身份问题**：怎样证明知识引用的是哪一版原文？
2. **授权问题**：怎样证明审核和发布针对的是同一份候选？
3. **一致性问题**：SQLite、文件和 Git 不能组成一个事务时怎样恢复？
4. **可信回答问题**：检索相关但证据不足时怎样阻止系统硬答？
...
主要风险不是功能缺失，而是信号被噪声淹没：

1. **赛道拥挤**：知识库、RAG、MCP 都是高频关键词，只报技术名很难区分。
2. **仓库偏重**：审计时 `demo` 占约 600 个受控文件，`demo/results` 约 41 MB；
   大量历史 Candidate 制品会增加克隆和评审成本。
3. **核心文件偏大**：`workspace.py`、`cli.py`、`query.py`、`compiler.py` 均超过
   2,000 行，陌生评审者难在短时间内定位职责。
4. **数字多但主证据不突出**：测试数、Benchmark、Candidate、平台门禁很多，容易给人
   “指标堆叠”印象。
5. **外部使用证据不足**：公开题集证明可复现，不等于证明真实用户愿意持续使用。
6. **发布口径分散**：Release、当前 `main`、development、confirmation、holdout 的数字
   不同，若不先说明固定 Commit，容易被认为口径混乱。
7. **能力面过宽**：CLI、Portal、Desktop、MCP、飞书、会话记忆、Code Wiki 同时出现，
   容易让核心主线失焦。
...
```mermaid
flowchart TD
    P0["P0：先让人看懂并相信"] --> A["一条零 Key 演示"]
    P0 --> B["当前 main 门禁全绿"]
    P0 --> C["三项核心指标"]
    P1["P1：补真实有效性"] --> D["3-5 位真实用户"]
    P1 --> E["失败案例闭环"]
    P1 --> F["仓库减重"]
    P2["P2：再降维护成本"] --> G["按变化热点拆大模块"]
    P2 --> H["补平台验证"]
    A --> P1
    B --> P1
    C --> P1
...
| 动作 | 交付物 | 验收标准 |
| --- | --- | --- |
| 固定一条主故事 | README 首屏只讲“来源 → 审核 → 发布 → 带证据回答” | 30 秒能复述，3 分钟能演示 |
| 固定零 Key Demo | 复用 `run_showcase_demo.py`，不要再造入口 | 新机器按文档一次跑通 |
| 保持当前 `main` 全绿 | Ruff、Mypy、Pytest、Registry、build | 输出绑定 Commit，不借用旧 Release 数字 |
| 补运行效率数据 | 冷启动、导入、Apply、查询的 p50/p95 和索引体积 | 固定机器、固定数据集、脚本可复现 |
| 准备一个失败演示 | stale ChangeSet、证据不足或恢复中任选一个 | 现场能解释“为什么拒绝继续” |

P0 的重点是**证据闭环**。不要继续增加 Provider、页面或 Agent。

#### P1：最能拉开秋招差距

| 动作 | 为什么重要 | 最小做法 |
| --- | --- | --- |
| 做真实用户评测 | 回答“除了作者谁会用” | 找 3-5 位同学，各完成一次导入、查询、更新 |
| 记录任务完成率 | 比 Star 数更能证明产品价值 | 统计首次成功率、完成时间、失败步骤 |
| 收敛仓库制品 | 降低“生成物堆积”观感 | 主分支保留最终 Evidence 和少量关键失败，完整制品放 Release |
| 统一版本口径 | 防止面试时数字互相冲突 | 所有结果同时写版本、Commit、平台、数据集 |
| 做一分钟录屏 | 降低评审者启动成本 | 展示导入、审核、Apply、带 Citation 回答 |

历史 Evidence 有审计价值，不应在没有归档方案时直接删除。先制定保留表，再把可下载的二进制
制品迁移到 GitHub Release；主分支保留摘要、哈希和必要负结果即可。

#### P2：有真实维护压力后再做

| 动作 | 触发条件 |
| --- | --- |
| 拆分 `workspace.py` | 某类存储改动持续触碰无关区域，或评审无法隔离职责 |
| 拆分 `query.py` | 路由、召回、证据和回答阶段无法独立测试或修改 |
| 拆分 `cli.py` | 新命令继续增长，启动时间或冲突明显恶化 |
| 增加语义召回通道 | 新的独立数据集持续证明词法召回不足 |
| 补 Windows 正式支持 | 有真实 Windows 用户，并能运行原生门禁 |

不要为了“代码行数好看”机械拆文件。拆分应跟随稳定职责和真实变化热点，否则只会增加跳转。

### 25.5 两周最小行动计划

| 时间 | 任务 | 结果 |
| --- | --- | --- |
| 第 1-2 天 | 跑完整本地门禁，修复当前 `main`，冻结演示 Commit | 一份可信的验证摘要 |
| 第 3 天 | 把零 Key Showcase 录成 60-90 秒视频 | README 可直接观看 |
| 第 4-5 天 | 增加冷启动、导入、查询和索引体积测量 | 一张性能表，不做无依据优化 |
| 第 6-9 天 | 邀请 3-5 位目标用户完成固定任务 | 用户完成率、耗时、阻塞点 |
| 第 10 天 | 只修最高频的一个阻塞点 | 一个前后对比案例 |
| 第 11-12 天 | 整理主分支 Evidence，统一版本口径 | 更轻的项目阅读路径 |
| 第 13 天 | 按第 21 节做两轮模拟追问 | 能解释取舍和失败边界 |
| 第 14 天 | 定稿简历与演示材料 | 代码、数字、表述使用同一 Commit |

### 25.6 建议补充的性能实验

正确性指标已经很多，下一步更需要系统指标：

| 指标 | 测量方式 | 能回答的问题 |
| --- | --- | --- |
| 冷启动时间 | 空缓存启动 Portal，重复 10 次 | 本地工具是否足够轻 |
| 导入吞吐 | 固定文件数和总字节数 | 来源规模增大后是否线性退化 |
| 增量编译比例 | 修改一个文件后统计重建页面数 | 是否真的避免全量重编译 |
| 查询 p50/p95 | 固定问题集，禁用模型与启用模型分别测 | 延迟来自检索还是模型 |
| 索引放大比 | SQLite + Blob + Wiki 体积 / 原文体积 | 本地存储代价 |
| Apply 恢复时间 | 在固定 Journal 阶段注入失败 | 恢复机制是否只有正确性、没有可用性 |

每个数字必须同时记录：MemoryForge Commit、数据集 Commit、Python 版本、平台、运行次数和
是否启用模型。没有这些条件，数字不应写进简历。

### 25.7 简历表述模板

不要写：

> 使用 Python、RAG、MCP 做了一个智能知识库，支持多种数据源和 AI 问答。

这种写法把最有价值的部分全部隐藏了。可以写成：

> 设计并实现本地优先的技术知识编译器，将 Git、文档和 AI 会话版本化为可审核 Markdown
> Wiki；通过不可变 ChangeSet、哈希绑定审批和 Apply Journal，保证跨 SQLite、文件系统与
> Git 的发布可追溯、可恢复。

> 基于 SQLite FTS5、结构化路由与 RRF 构建渐进式检索，在固定 30 题公开集上将 Top-3
> 来源召回从 Raw FTS 的 57.7% 提升到 96.2%，并保留外部 Click 数据集 0% holdout
> 准确率这一负结果，明确系统泛化边界。

> 使用 Tree-sitter 统一抽取 Python、Go、TypeScript 的 Symbol 与 Relation，生成带源码
> Citation 的 Code Wiki 和 Mermaid 架构图；通过 Pydantic、Mypy、Ruff、Pytest 和
> clean-room 构建验证数据合同与发布制品。

第一条证明系统设计，第二条证明实验能力，第三条证明代码分析和工程质量。简历只保留
两到三条，不要把全部功能都塞进去。

### 25.8 一分钟面试介绍

> 很多 AI 知识库只解决“怎样检索”，但没有解决“知识怎样可信地进入索引”。我做的
> MemoryForge 在 RAG 前增加了一层知识编译流程：先把代码和文档保存成不可变来源版本，
> 再生成 ChangeSet；审核和授权都绑定候选哈希，Apply 时重验 Git 基线与来源版本，并用
> Journal 处理 SQLite、文件和 Git 之间的中断。查询侧把相关性和证据充分性分开，证据不足
> 就返回 partial 或 no_local_evidence。项目最核心的三个点是版本身份、跨介质恢复和
> grounded answer，而不是模型 API。

说完后立即演示一条成功链路和一条 fail-closed 链路，比继续罗列功能更有效。
...
- 为了显得“AI”而强行加入向量数据库；
- 把单机工具拆成微服务、消息队列或 Kubernetes；
- 再接更多模型、聊天平台或 Agent 框架；
- 为了测试数量继续添加重复用例；
- 在没有用户反馈时重写 Portal 前端；
- 隐藏 Click 外部评测失败或把固定夹具的 100% 外推到真实仓库。

这些改动会扩大范围，却不会增强项目最重要的证据。

### 25.10 投递前 Definition of Done

- README 首屏能在 30 秒内回答“是什么、为什么不是普通 RAG、最硬的结果是什么”；
- 一条零 Key 命令能跑出可看的完整流程；
- 当前 Commit 的 Ruff、Mypy、Pytest、Registry 和构建门禁有可核验结果；
- 至少有一组性能数据和一组真实用户任务数据；
- 所有百分比都绑定数据集、Commit 和边界；
- 能现场解释一次成功发布、一次 stale 拒绝和一次证据不足拒答；
- 简历只写两到三条最强证据，不罗列所有技术名词；
- 不把 `local_only`、Windows 支持或 Benchmark 能力说得超过实际边界。

达到这些条件后，MemoryForge 的短板就不再是“项目普通”，而主要是外部采用规模。对秋招项目
来说，这是比继续扩功能更值得投入的方向。
